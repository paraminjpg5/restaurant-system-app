...
async function seed() {
  ...
  // Create categories
  const categoryResults = await db.insert(schema.categories).values([
    {
      name: "ก๋วยเตี๋ยว",
  ...
  const sweetnessOption = await db.insert(schema.customizationOptions).values({
    name: "ระดับความหวาน",
    type: "sweetness",
  });

  const toppingOption = await db.insert(schema.customizationOptions).values({
    name: "ท็อปปิ้ง",
    type: "topping",
  });
  ...
  // Create menu items
  const menuItems = await db.insert(schema.menuItems).values([
    {
      categoryId: 1,
      name: "ก๋วยเตี๋ยวน้อย",
